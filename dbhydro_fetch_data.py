...
def fetch_station_data(station, config, logger):
    """Name:       fetch_station_data.
    ...
    logger.info('fetch_station_data(): Fetching %s' % station)
    try:
        resp = requests.get(url)
    except requests.ConnectionError as error:
        logger.info("fetch_station_data() Failed with error %s" % error)
    # Consider any status other than 2xx an error
    if not resp.status_code // 100 == 2:
        logger.info('fetch_station_data(): Unexpected response {}'.format(resp))
        return False
    data_frame = pd.read_csv(io.StringIO(resp.content.decode('utf-8')))
    return data_frame


def extract_station_data(station, data_frame, config, logger):
    """Name:       extract_station_data.
    ...
    """
    logger.info("extract_station_data(%s)" % station)
    dbhvars = config['variables']
    new_df = data_frame[dbhvars].copy().dropna()
    return new_df


def process_stations(logger):
    ...
    for site in config['stations']:
        ...
        data_frame = fetch_station_data(station, config, logger)
        extracted_df = extract_station_data(station, data_frame, config, logger)
        if len(extracted_df) > 0:
            filename = "reports/%s.csv" % station
            logger.info('process_stations(): Writing %s' % filename)
            csv = extracted_df.to_csv(filename, index=False)
        else:
            logger.warning('process_stations(): %s returned no data.' % station)


if __name__ == '__main__':
    logger = config_logging()
    process_stations(logger)
